...
/**
 * A simple command-line utility to rename an MP3 file based on its ID3v1 title tag.
 *
 * How It Works:
 ...
 * 2. Run from the command line: java MP3Renamer "<path_to_your_song.mp3>"
 */
public class MP3Renamer {
    ...
    public static void main(String[] args) {
        // Ensure the user has provided a file path as a command-line argument.
        if (args.length < 1) {
            System.err.println("Usage: java MP3Renamer \"<path_to_mp3_file>\"");
            return;
        }
        ...
        try {
            // Attempt to read the ID3 title from the file.
            ...
            if (title != null && !title.isEmpty()) {
                ...
                if (sanitizedTitle.isEmpty()) {
                    System.err.println("Error: The extracted title contains only invalid filename characters.");
                    return;
                }
                ...
                if (newFile.exists()) {
                    System.err.println("Error: A file named \"" + newFile.getName() + "\" already exists in this folder.");
                    return;
                }

                // Perform the rename operation.
                if (mp3File.renameTo(newFile)) {
                    System.out.println("Successfully renamed '" + mp3File.getName() + "' to '" + newFile.getName() + "'");
                } else {
                    System.err.println("Error: Failed to rename the file.");
                }

            } else {
                System.out.println("No valid ID3v1 title tag found in the file.");
            }
        } catch (IOException e) {
